...
def test_count_tokens_fallback(mock_tiktoken):
    ...
    mock_fallback_encoding.encode.return_value = [
        1,
        2,
        3,
    ]  # Simulate encoding to 3 tokens.
    ...
    mock_fallback_encoding.encode.assert_called_once_with(
        text_to_encode, errors="replace"
    )
...
@patch(
    "builtins.open", new_callable=mock_open, read_data='{"saved_paths": ["/test/path"]}'
)
def test_load_config_exists(mock_file, mock_config_path):
    ...
    # Note: We can't easily assert the exact call to open(CONFIG_FILE) because CONFIG_FILE is a mock now  # noqa: E501
