...
def test_fetch_data(mock_fetch, audit_engine, mock_console):
    ...
    assert any("Fetching" in str(arg) for args, _ in mock_console.print.call_args_list for arg in args)
...
def test_build_prompt(audit_engine, mock_template, mock_commit_data):
    ...
    with patch("src.engine.count_tokens", return_value=100):
...
def test_execute_ai_stream(mock_live, mock_settings, mock_get_provider, audit_engine):
