...
# --- UPDATED IMPORTS: Get constants from their source ---
from src.constants import OPT_STAGED
from src.cli import (
    get_repository_path,
...
def test_handle_raw_extraction_success(mock_console, mock_tui, mock_engine):
    """Test that CLI delegates raw extraction to the engine using the new mapping."""
    # Setup CLI inputs
    # Crucial: Use the constant so EXTRACTION_MODE_MAPPING finds the key
    ...
    # args[0] = repo_path, args[1] = filters, args[2] = full_output_path
    assert call_args[0][0] == "/repo"
    assert call_args[0][1] == {"mode": "staged"}

    # Verify path mapping logic worked: OPT_STAGED maps to "Staged_Changes"
    assert "Staged_Changes" in call_args[0][2]
...
def test_run_app_flow(
    ...
    # We use the raw string here because Main Menu choices in run_app
    # are still string literals (we only refactored the submenu options).
