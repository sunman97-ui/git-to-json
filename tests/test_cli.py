...
def test_handle_raw_extraction_success(mock_console, mock_tui, mock_engine):
    ...
    # We look for "Saved to" because that's what the actual code prints: "✅ Saved to {path}"
    ...
    assert found_success_msg, f"Success message not found. Console calls: {mock_console.print.call_args_list}"
...
def test_handle_interactive_workflow_ai(mock_tui, mock_engine):
    ...
    mock_engine.fetch_data.assert_called_with("/repo", {"mode": "hashes", "hashes": ["hash1"]})
    mock_engine.build_prompt.assert_called()
...
@patch("src.cli.console")
@patch("src.cli.load_config") 
def test_run_app_flow(mock_cfg, mock_console, mock_get_repo, mock_tui, mock_load_tmpl, mock_raw):
