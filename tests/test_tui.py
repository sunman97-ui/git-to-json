...
def test_select_commits_interactively_success(mock_get_commits, mock_questionary_checkbox):
    """Test successful interactive commit selection."""
    mock_get_commits.return_value = [
        {"hash": "hash1", "short_hash": "abc", "date": "2023-01-01", "author": "Author1", "message": "Msg1", "files": ["f1"]},
        {"hash": "hash2", "short_hash": "def", "date": "2023-01-02", "author": "Author2", "message": "Msg2", "files": ["f2", "f3"]}
    ]
...
def test_get_main_menu_choice_interactive_option(mock_questionary_select):
    ...
    assert choices.index("🤝 Interactive Commit Selection") > choices.index(mock_template.meta.name)
    assert choices.index("🤝 Interactive Commit Selection") < choices.index("🚀 Execute AI Prompt (Direct Mode)")